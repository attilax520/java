...
String stdout_2str_ByIoutil = null;
		//		Process Process1 = ProcessUtil.start(cmd, null);
//
//		String stdout_2str_ByIoutil = CliShellProcessService.stdout_2str_ByIoutil(Process1, charsetName);
//		String erroutput2strByIoutil = CliShellProcessService.Erroutput2strByIoutil(Process1, charsetName);
//		System.out.println(erroutput2strByIoutil);
	...
	public static int parseMemusePercent(List<String> result_tmp) {
		for (String line : result_tmp) {
			line = line.trim();
			if (line.trim().toLowerCase().startsWith("kib mem")) {
				int memAll =memMonitorLinux .getmemallLinux(line);
			   int	memCanuse =memMonitorLinux. parseCanuseMemLinux(line);
			   int usemem=memAll-memCanuse;
			    int useRate = ((Double) (  (double)usemem/(double)memAll*100  )).intValue();
			    return useRate;
			}
		}
	...
	// Mem: 1939780k total, 1375280k used,
	public static int parseCanuseMemLinux(String line) {
...
//		use = use.substring(stat2);
	...
	// Mem: 1939780k total, 1375280k used,
	public static  int getmemallLinux(String line) {
		int stat = line.indexOf(":");
