...
import com.kok.sport.integration.impl.*;
...
public class FootballMatchController implements IFootballMatchController {
    ...
    @Autowired
    public SyncFootballEuropeOddseuropeliveImp syncFootballEuropeOddseuropeliveImp;
    @Autowired
    public SyncFootballNumberOddsHistoryServiceImpl syncFootballNumberOddsHistoryService;
    @Autowired
    public SyncFootballBasicUpdateprofileServiImp syncFootballBasicUpdateprofileServiImp;
    ...
    public Object FootballMatchService(String service) throws Exception {
        ...
        } else if ("Football.Basic.Update_profile".equals(service)){//gray改动

            syncFootballBasicUpdateprofileServiImp.Football_Basic_Update_profile();

        } else if ("Football.Europe.Odds_europe_history".equals(service)){//gray改动

            syncFootballEuropeOddseuropeliveImp.Football_Europe_Odds_europe_live();

        } else if ("Football.Number.Odds_history".equals(service)){//gray改动

            syncFootballNumberOddsHistoryService.insertFootballNumberOddsData(null);
