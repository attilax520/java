	...
	public void jincyoTonji_Football_Live_Match_detail_live() throws Exception {
		...
		for (Map MatchItem : result1) {
			...
			try {
				...
				tonzhi_processtliveList(tliveList, session, MatchItem);
			} catch (Exception e) {
				logger.warn(e);
				// dbgExit(e);
			}
...
//replace(UNIX_TIMESTAMP(now(6)),'.','')
	...
	private static void sendTongzhi(Map matchItem, SqlSession session2) {
		...
		try {
			...
			// String matchid= mid.intValue();

			String sql = "select * from football_tlive_v where match_id=" + mid.intValue();
			logger.info(sql);
