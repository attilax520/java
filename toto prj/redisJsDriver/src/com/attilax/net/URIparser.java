...
//		int pathStartcharIndex=this.input.lastIndexOf("/");
//		int queryStart=this.input.lastIndexOf("?");
//		return this.input.substring(pathStartcharIndex+1,queryStart);
	//	return path;
	...
	private String decode(String fragment2) {
...
//	}
	...
	class Parser {
		...
		//		fragment = substring(p + 1, n);
			...
			path=getPath(this.input);
			decodedPath=path;
			path_final=path;
			
			decodedQuery=getQuery(this.input);
				System.out.println("");
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		private String getQuery(String input2) {
			int queryStart=input2.lastIndexOf("?");
			String substring = input2.substring(queryStart+1);
			return substring;
		}

		private String getPath(String input2) {
			int pathStartcharIndex=input2.lastIndexOf("/");
			int queryStart=input2.lastIndexOf("?");
			
			if(pathStartcharIndex<queryStart )
			{
			String substring = input2.substring(pathStartcharIndex,queryStart);
			return substring;
			}else {
				return "";
			}
		...
		private int getPort(String input2) {
			if(hasUserinfo(input2)) {
				...
				if(hasPath(input2) )
				{
					...
					return  port;
				}else if(hasQuery(input2)) { // with no path
					int queryStart = input.indexOf("?");
					port =   Integer.parseInt(input.substring(portSplitorPos + 1, queryStart));
					return  port;
				}
	...
	private boolean hasQuery(String input2) {
			 
			return input.indexOf("?")>-1;
		}

	private boolean hasPath(String input2) {
		int syegeoStart = input.indexOf("//");
		
		
	    
		int pathStart = input.indexOf("/", syegeoStart + 2);
		int venhao = input.indexOf("?");
		if(venhao==-1)  //query not exist		 
			return pathStart>-1;
			else  //query exist  (venhao>-1)
		{
			if(pathStart>-1 && venhao>pathStart )   
				return true;
			else {
				return false;
			}
