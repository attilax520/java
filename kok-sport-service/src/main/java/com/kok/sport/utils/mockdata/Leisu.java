		...
		for (Element tr : trs) {
			...
			if (tr.hasAttr("data-id")) {
				try {
					...
					String sql = "insert into mat_runtime set  match_id= " + matid + ",min=" + time_min_int.toString()
							+ ",minstr='" + timestr_min + "',`match`=" + match;
					sql=sql+" ON DUPLICATE KEY UPDATE half_court_score=values(half_court_score)";
